function [W,H] = nmfsc( V, rdim, NumLab, SmpClass, A, sW, sH, fname)
...
Z = abs(rand(samples+SmpClass-NumLab,rdim));
H=A*Z;
H=H';
H = H./(sqrt(sum(H.^2,2))*ones(1,samples));     %用以归一化

% 注意此时需要注意矩阵之间的转置，原始公式中是有转置的
% H = abs(randn(rdim,samples));
% H = H./(sqrt(sum(H.^2,2))*ones(1,samples));

% Make initial matrices have correct sparseness
% 以下判断中，如果sW或者sH是空的话就不执行判断体内容，
% 如果不是空是具有值的话就执行判断体中的内容
if ~isempty(sW), 
